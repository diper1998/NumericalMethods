...
void MainWindow::Do(){
    ...
    difEq.c = ui->lineEdit_c->text().toDouble();
    difEq.d = ui->lineEdit_d->text().toDouble();
    difEq.M = ui->lineEdit_M->text().toDouble();
...
if(ui->checkBox_End->isChecked()){
 difEq.end = true;
    ...
    ui->label_mine1->setText("Min(e1): " + difEq.arrayMaxMin[1].toString()+" при x="+difEq.arrayMaxMin[3].toString());
     ...
     countDiv = difEq.countDiv;
     countDoub = difEq.countDoub;

     ui->label_divh->setText("Делений h: " + countDiv.toString());
     ui->label_mulh->setText("Удвоений h: " + countDoub.toString());
